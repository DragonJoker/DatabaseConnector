/************************************************************************//**
 * @file DatabaseResultMySql.cpp
 * @author Sylvain Doremus
 * @version 1.0
 * @date 3/20/2014 2:47:39 PM
 *
 *
 * @brief CDatabaseResultMySql class definition.
 *
...
{
	static const String   MYSQL_DRIVER_ERROR = STR( "MYSQL Driver error : " );
	static const String   MYSQL_UNKNOWN_ERROR = STR( "Unknown error encountered while executing query" );
	static const String   MYSQL_INCONSISTENCY_ERROR = STR( "Number of columns is less than retrieved data." );

	CDatabaseResultMySql::CDatabaseResultMySql( DatabaseConnectionPtr connection, std::shared_ptr< sql::Statement > statement )
	...
	CDatabaseResultMySql::~CDatabaseResultMySql()
	{
	...
	void CDatabaseResultMySql::Initialize( std::shared_ptr< sql::ResultSet > result )
	{
		_result = result;
	...
	DatabaseRowPtr CDatabaseResultMySql::GetFirstRow()
	{
	...
	DatabaseRowPtr CDatabaseResultMySql::GetNextRow()
	{
		DatabaseRowPtr result;
	...
	void CDatabaseResultMySql::DoGetRow()
	{
		bool null = true;
		...
		try
		{
			...
			{
				sql::SQLString fieldValue;
				MySQLTry( null = _result->isNull( i ), STR( "Is parameter null retrieval" ) );
		...
		catch ( const CExceptionDatabase & e )
		{
			...
			message << MYSQL_DRIVER_ERROR << STR( " - " )
					<< e.what();
