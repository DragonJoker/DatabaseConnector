...
BEGIN_NAMESPACE_DATABASE_ODBC
{
    ...
    EErrorType CDatabaseStatementOdbc::Initialize()
    {
        ...
        if ( errorType == EErrorType_NONE )
        {
            SqlTry( SQLPrepare( _statementHandle, ( SqlChar * )_query.c_str(), SQLINTEGER( _query.size() ) ), SQL_HANDLE_STMT, _statementHandle, ODBC_PREPARE_MSG );
        }
    ...
    DatabaseParameterPtr CDatabaseStatementOdbc::CreateParameter( const String & name, EFieldType fieldType, EParameterType parameterType )
    {
        ...
        DatabaseParameterPtr pReturn( new CDatabaseStatementParameterOdbc( std::static_pointer_cast< CDatabaseConnectionOdbc >( _connectionOdbc ), name, ( unsigned short )_arrayParams.size() + 1, fieldType, parameterType, new SValueUpdater( this ) ) );
    ...
    {
        // std::make_shared limited to 5 parameters with VS2012
        DatabaseParameterPtr pReturn( new CDatabaseStatementParameterOdbc( std::static_pointer_cast< CDatabaseConnectionOdbc >( _connectionOdbc ), name, ( unsigned short )_arrayParams.size() + 1, fieldType, limits, parameterType, new SValueUpdater( this ) ) );
