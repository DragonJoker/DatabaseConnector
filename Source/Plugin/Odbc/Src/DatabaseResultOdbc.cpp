...
BEGIN_NAMESPACE_DATABASE_ODBC
{
    ...
    void CDatabaseResultOdbc::Initialize( HSTMT statementHandle )
    {
        ...
        if ( errorType == EErrorType_NONE && numColumns )
        {
            ...
            {
                SqlTry( SQLColAttribute( _statementHandle, i + 1, SQL_DESC_LABEL, SQLPOINTER( buffer ), BUFFER_SIZE, &numResults, &numeric ), SQL_HANDLE_STMT, _statementHandle, ODBC_COLATTRIBUTE_DESCLABEL_MSG );
                ...
                if ( errorType == EErrorType_NONE )
                {
                    ...
                    SqlTry( SQLColAttribute( _statementHandle, i + 1, SQL_DESC_CONCISE_TYPE, SQLPOINTER( buffer ), BUFFER_SIZE, &numResults, &numeric ), SQL_HANDLE_STMT, _statementHandle, ODBC_COLATTRIBUTE_DESCTYPE_MSG );
                    
                    if ( errorType == EErrorType_NONE )
                    {
                        if ( numeric == SQL_NTS )
                        {
                            SqlTry( SQLColAttribute( _statementHandle, i + 1, SQL_DESC_TYPE_NAME, SQLPOINTER( buffer ), BUFFER_SIZE, &numResults, &numeric ), SQL_HANDLE_STMT, _statementHandle, ODBC_COLATTRIBUTE_TYPENAME_MSG );
