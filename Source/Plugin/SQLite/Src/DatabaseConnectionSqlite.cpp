...
BEGIN_NAMESPACE_DATABASE_SQLITE
{
    ...
    bool CDatabaseConnectionSqlite::ExecuteUpdate( std::shared_ptr< SQLite::Statement > statement )
    {
        bool result = false;
        
        try
        {
            SQLiteTry( statement->executeUpdate(), STR( "Statement execution" ) );
            result = true;
        }
    ...
    DatabaseResultSqlitePtr CDatabaseConnectionSqlite::ExecuteSelect( std::shared_ptr< SQLite::Statement > statement )
    {
        DatabaseResultSqlitePtr result;
        
        try
        {
            bool hasResults = false;
            std::shared_ptr< sql::ResultSet > rs;
            SQLiteTry( rs.reset( statement->executeQuery() ), STR( "Statement execution" ) );
    ...
    bool CDatabaseConnectionSqlite::ExecuteUpdate( const String & query )
    {
    ...
    DatabaseResultSqlitePtr CDatabaseConnectionSqlite::ExecuteSelect( const String & query )
    {
    ...
    EErrorType CDatabaseConnectionSqlite::DoConnect( String & connectionString )
    {
        ...
        CStrUtils::Replace( serverPath, _T( "\\" ), PATH_SEP );
        CStrUtils::Replace( serverPath, _T( "/" ), PATH_SEP );
        ...
        {
            connectionString = _T( "Erreur de connexion à la base de données : \n" );
    ...
    DatabaseResultSqlitePtr CDatabaseConnectionSqlite::DoRetrieveResults( std::shared_ptr< SQLite::Statement > statement )
    {
        DatabaseResultSqlitePtr pResult;
        
        try
        {
            pResult = std::make_shared< CDatabaseResultSqlite >( shared_from_this(), statement );
            pResult->Initialize();
        }
        catch ( std::exception & exc )
        {
            StringStream stream;
            stream << ERROR_SQLITE_EXECUTION_ERROR << exc.what();
